...
rho_wdro    =  10*np.sqrt(2.0)/np.sqrt(250)
rho_skwdro  =  np.sqrt(2.0)/np.sqrt(250)
...
def choose_threshold_regression(f, X_cal, y_cal, q0, target_risk, delta, bound_type, grid_t):
    ...
    for t in grid_t:
        ...
        if bound_type in ["Hoeffding", "Bernstein"]:
            ...
            bnd = WDRO_special_bound(residuals_cal, q0 + t, n, rho=rho_wdro)
            cal_risks.append(r_hat)
            bounds.append(bnd - r_hat)
            upper_bounds.append(bnd)
        elif bound_type == "SKWDRO":
            bnd = WDRO_entropy_bound(f, X_cal, y_cal, q0 + t, n, rho=rho_skwdro)
            cal_risks.append(r_hat)
            bounds.append(bnd - r_hat)
            upper_bounds.append(bnd)
        else:
...
def plot_distribution_comparisons(rep_results, bound_types, delta, target_risk):
    ...
    """
    metrics = [ "coverage", "avg_interval_size"]
    titles = {
       # "t_chosen": "Chosen Extra Buffer t",
        "coverage": "Test Coverage (1 - Risk)",
    ...
    for i, metric in enumerate(metrics):
        ...
        axs[i].boxplot(data, tick_labels=bound_types,whis=(delta,1-delta))
    ...
    plt.savefig(folder+"Results.pdf")
    #plt.show()
...
def main():
    ...
    nominal_alpha = target_risk     # Base miscoverage level
    delta = 0.05                     # Confidence parameter for the concentration bound
    bound_types = ["Hoeffding", "Bernstein", "WDRO" , "SKWDRO"]
    grid_t = np.linspace(0, 50, 101)  # Extended grid for t
    
    results_example, model = simulation_run_regression(n_samples=n_samples, nominal_alpha=nominal_alpha,
    ...
    rep_results = {btype: {"t_chosen": [], "coverage": [], "avg_interval_size": []}
                   for btype in bound_types}
    
    for rep in range(n_replications):
        if int(100*rep/n_replications)%10 == 0:
            print(f"{rep+1}/{n_replications} runs")
        sim_res, _ = simulation_run_regression(n_samples=n_samples, nominal_alpha=nominal_alpha,
                                                target_risk=target_risk, delta=delta,
                                                bound_types=bound_types, grid_t=grid_t)
        for btype in bound_types:
